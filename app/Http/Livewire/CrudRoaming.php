...
class CrudRoaming extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    {
        $products = Roaming::orderBy("id", "desc")->paginate(10);
        $dataAgents = Agent::all();
        $dataMonths = Month::all();
        return view('livewire.crud-roaming', ['products' => $products, 'dataAgents' => $dataAgents, 'dataMonths' => $dataMonths])->layout('layouts.base');
    }
    ...
    public function store()
    {
        ...
        $dataproduct = Roaming::updateOrCreate(
            ['id' => $this->productid],
            [
                'sku' => $lastskuvalue,
                'name' => $this->name,
        ...
        DB::table('agent_roaming')->where('id_package', $this->productid)->delete();

        $i = 1;
        foreach ($this->agents as $agent) {
            $dataagentproduct = Agentroaming::updateOrCreate(
                ['id' => $this->agentproductid],
                [
                'id_agent' => $agent,
                'id_package' => $dataproduct->id,
    ...
    {
        Roaming::find($this->deleteId)->delete();
